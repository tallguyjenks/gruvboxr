...
gnome_theme_dark_backup <- function() {
  fs::path(get_stylesheets_location(), "rstudio-gnome-dark-gruvboxr-backup.qss")
}
...
windows_theme_dark_backup <- function() {
  fs::path(get_stylesheets_location(),"rstudio-windows-dark-gruvboxr-backup.qss")
}
...
gruvboxr_installed <- function() {
 !is.null(rstudioapi::getThemes()$gruvboxr)
}
